...
(function (global, factory) {
    ...
    class CssAccordion {
      ...
      init() {
        ...
        this.items.forEach(details => {
          ...
          const panel = details.querySelector('.nsw-accordion__content-wrap') || details.querySelector('.nsw-accordion__content');
        ...
        if (location && location.hash) {
          ...
          const details = scoped && (byId.matches && byId.matches('details.nsw-accordion__item') ? byId : byId.closest('details.nsw-accordion__item'));
          if (details) {
            if (!details.open) {
              details.open = true;
              if (updateButtons) updateButtons();
  ...
  `;
      ...
      <div class="nsw-cookie-dialog nsw-dialog nsw-dialog--single-action js-dialog js-dialog-dismiss" id="cookie-consent-preferences" role="dialog" aria-modal="true" aria-hidden="true" aria-labelledby="cookie-dialog-title">
    ...
    function getKeyboardHintId(node) {
      ...
      let hintId = keyboardHintIdCount === 1 ? keyboardHintIdBase : `${keyboardHintIdBase}-${keyboardHintIdCount}`;
    ...
    class QuickExit {
      ...
      static enhance(root, {
        ...
        let desc = contentDiv.querySelector('.nsw-quick-exit__description-text') || node.querySelector('.nsw-quick-exit__description-text');
        if (enableEsc) {
          if (desc) desc.remove();
      ...
      static autoInit() {
        ...
        const hasStickyQuickExit = document.querySelector('.nsw-sticky-container .nsw-quick-exit, .js-sticky-container .nsw-quick-exit, #sticky-container .nsw-quick-exit');
    ...
    function initSite() {
      ...
      const detailsAccordions = Array.from(document.querySelectorAll('.js-accordion-details')).filter(element => !element.parentElement || !element.parentElement.closest('.js-accordion-details'));
...
;(function(g){try{g.NSW=g.NSW||{};g.NSW.VERSION="3.24.15";}catch(e){} }(typeof globalThis!=='undefined'?globalThis:(typeof window!=='undefined'?window:self)));