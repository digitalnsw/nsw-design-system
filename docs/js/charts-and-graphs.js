/*! NSW Design System v3.24.10 | MIT License */
(function (factory) {
  ...
  const parseRgbColor = raw => {
    ...
    const parts = match[1].replace(/\s*\/\s*/g, ',').replace(/\s+/g, ',').split(',').map(part => part.trim()).filter(Boolean);
  ...
  const getContrastAtAlpha = (baseColor, inkColor, alpha) => {
    ...
    return getContrastRatio(getRelativeLuminance(base), getRelativeLuminance(blended));
  };
  ...
  const defaultNswInkIndex = Object.freeze({
    ...
    '--nsw-aboriginal-palette-earth-red': {
      baseColor: '#950906',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-ember-red': {
      baseColor: '#e1261c',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-coral-pink': {
      baseColor: '#fbb4b3',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-galah-pink': {
      baseColor: '#fdd9d9',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-deep-orange': {
      baseColor: '#882600',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-orange-ochre': {
      baseColor: '#ee6314',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-clay-orange': {
      baseColor: '#f4aa7d',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-sunset-orange': {
      baseColor: '#f9d4be',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-bush-honey-yellow': {
      baseColor: '#895e00',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-sandstone-yellow': {
      baseColor: '#fea927',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-golden-wattle-yellow': {
      baseColor: '#fee48c',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-sunbeam-yellow': {
      baseColor: '#fff1c5',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-riverbed-brown': {
      baseColor: '#552105',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-firewood-brown': {
      baseColor: '#9e5332',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-claystone-brown': {
      baseColor: '#d39165',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-macadamia-brown': {
      baseColor: '#e9c8b2',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-charcoal-grey': {
      baseColor: '#272727',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-emu-grey': {
      baseColor: '#555555',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-ash-grey': {
      baseColor: '#ccc6c2',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-smoke-grey': {
      baseColor: '#e5e3e0',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-bushland-green': {
      baseColor: '#215834',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-marshland-lime': {
      baseColor: '#78a146',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-gumleaf-green': {
      baseColor: '#b5cda4',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-saltbush-green': {
      baseColor: '#dae6d1',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-billabong-blue': {
      baseColor: '#00405e',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-saltwater-blue': {
      baseColor: '#0d6791',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-light-water-blue': {
      baseColor: '#84c5d1',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-coastal-blue': {
      baseColor: '#c1e2e8',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-bush-plum': {
      baseColor: '#472642',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-spirit-lilac': {
      baseColor: '#9a5e93',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-lilli-pilli-purple': {
      baseColor: '#c99ac2',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-dusk-purple': {
      baseColor: '#e4cce0',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-black': {
      baseColor: '#000000',
      inkColor: '#ffffff'
    },
    '--nsw-aboriginal-palette-off-white': {
      baseColor: '#f2f2f2',
      inkColor: '#495054'
    },
    '--nsw-aboriginal-palette-white': {
      baseColor: '#ffffff',
      inkColor: '#495054'
    }
  });
  const defaultInkCandidates = Object.freeze(['#ffffff', '#22272b', '#495054', '#002664', '#cbedfd']);
  ...
  const getNswChartPalette = (options = {}) => getPaletteByTokens(options, nswChartPaletteTokens, defaultNswChartPalette);
  const getNswAboriginalChartPalette = (options = {}) => getPaletteByTokens(options, nswAboriginalChartPaletteTokens, defaultNswAboriginalChartPalette);
  ...
  const normaliseInkIndexEntry = (key, entry, cssVariables) => {
    ...
    if (!baseColor) {
      baseColor = key.startsWith('--') ? getCssVariableColor(cssVariables, key) : key;
    }
  ...
  const getAccessibleInk = (baseColor, options = {}) => {
    ...
    if (baseHex && inkIndex && typeof inkIndex === 'object') {
      ...
      Object.keys(inkIndex).forEach(key => {
        ...
        const requiredContrast = Math.max(contrastThreshold, Number.isFinite(resolved.minContrast) ? resolved.minContrast : 0);
  ...
  const getPatternInkAlpha = (baseColor, inkColor, style, options = {}) => {
    ...
    const styleAlpha = Number.isFinite(baseAlphaByStyle[style]) ? baseAlphaByStyle[style] : baseAlphaByStyle.default;
  ...
  const createPattern = (input, options = {}) => {
    ...
    const resolvedCssVariables = resolveCssVariables(cssVariables, cssScope);
    const resolvedBaseColor = resolveBaseColor(baseColor, resolvedCssVariables);
    const resolvedMinPatternAlpha = clamp(Number.isFinite(minPatternAlpha) ? minPatternAlpha : 0.35, 0, 1);
    const resolvedMaxPatternAlpha = clamp(Number.isFinite(maxPatternAlpha) ? maxPatternAlpha : 1, resolvedMinPatternAlpha, 1);
    if (!targetContext || !resolvedBaseColor) return baseColor;
    const resolvedInk = inkColor === 'auto' ? getAccessibleInk(resolvedBaseColor, {
      inkIndex,
      inkCandidates,
      minContrast,
      enforceContrast,
    ...
    const resolvedInkAlpha = Number.isFinite(inkAlpha) ? clamp(inkAlpha, resolvedMinPatternAlpha, resolvedMaxPatternAlpha) : getPatternInkAlpha(resolvedBaseColor, resolvedInk, style, {
      minContrast,
      baseAlphaByStyle,
    ...
    const resolvedPatternSources = patternSources && typeof patternSources === 'object' ? patternSources : defaultPatternSources;
    ...
    const patternCanvas = typeof OffscreenCanvas !== 'undefined' ? new OffscreenCanvas(pixelWidth, pixelWidth) : document.createElement('canvas');
    ...
    if (tintSvg) {
      const overlayCanvas = typeof OffscreenCanvas !== 'undefined' ? new OffscreenCanvas(pixelWidth, pixelWidth) : document.createElement('canvas');
  ...
  function loadScriptOnce(scriptSource) {
    const source = typeof scriptSource === 'string' ? {
      src: scriptSource
    } : scriptSource;
    const {
      src,
  ...
  function initChartsAndGraphs() {
    ...
    if (!chartTargets.length) {
      chartTargets = document.querySelectorAll('canvas[id^="chart"], canvas[id^="example"], #bar, #pie');
    }
    if (!chartTargets.length) return;
    const renderCharts = () => {
      if (!window.Chart) return;
      const palette = getNswChartPalette({
        cssScope: document.body
      });
      ...
      const normaliseDirectLabelLines = value => {
        if (Array.isArray(value)) {
          return value.filter(line => line != null && line !== '').map(line => `${line}`);
        }
      ...
      const directSegmentLabelsPlugin = {
        ...
        afterDatasetsDraw: (chart, _args, pluginOptions = {}) => {
          ...
          const labelLineHeight = Number.isFinite(pluginOptions.labelLineHeight) ? pluginOptions.labelLineHeight : 14;
          const valueLineHeight = Number.isFinite(pluginOptions.valueLineHeight) ? pluginOptions.valueLineHeight : 14;
          ...
          const radialOffset = Number.isFinite(pluginOptions.radialOffset) ? pluginOptions.radialOffset : 18;
          const canvasMargin = Number.isFinite(pluginOptions.canvasMargin) ? pluginOptions.canvasMargin : 14;
          const labelPadding = Number.isFinite(pluginOptions.labelPadding) ? pluginOptions.labelPadding : 8;
          const leaderLineWidth = Number.isFinite(pluginOptions.leaderLineWidth) ? pluginOptions.leaderLineWidth : 1.5;
          ...
          meta.data.forEach((arc, dataIndex) => {
            ...
            const lines = normaliseDirectLabelLines(typeof pluginOptions.formatter === 'function' ? pluginOptions.formatter({
              chart,
              dataset,
              datasetIndex,
              dataIndex,
            ...
            const textWidth = lines.reduce((max, line, index) => Math.max(max, measureLineWidth(line, index)), 0);
            const textHeight = lines.reduce((height, _line, index) => height + (index === 0 ? labelLineHeight : valueLineHeight), 0) + (lines.length > 1 ? lineGap * (lines.length - 1) : 0);
          ...
          const clampLabelsWithinCanvas = items => {
            ...
            const sortedItems = [...items].sort((leftItem, rightItem) => leftItem.targetY - rightItem.targetY).map(item => ({
              ...item,
          ...
          Object.values(itemsBySide).forEach(items => {
            items.forEach(item => {
              const lineEndX = item.side === 'right' ? chart.width - canvasMargin - item.textWidth - labelPadding : canvasMargin + item.textWidth + labelPadding;
              ctx.beginPath();
              ...
              const textX = item.side === 'right' ? lineEndX + labelPadding : lineEndX - labelPadding;
              let lineY = item.resolvedY - item.textHeight / 2;
              item.lines.forEach((line, index) => {
                ctx.font = index === 0 ? labelFont : valueFont;
                ctx.fillStyle = index === 0 ? pluginOptions.labelColor || textDark : pluginOptions.valueColor || textDark;
      ...
      const pointStyleLegendLabels = chart => {
        ...
        return defaults.map(item => {
          ...
          const legendPointStyle = isTrianglePointStyle(pointStyle) ? getLegendTrianglePointStyle(fillStyle) : pointStyle;
          return {
            ...item,
      ...
      const createChart = (canvasId, config) => {
        ...
        if (chartPanel) {
          ...
          const calloutItems = Array.isArray(callout.items) ? callout.items.map(item => ({
            label: toHeadingText(item && item.label),
            value: toHeadingText(item && item.value)
          })).filter(item => item.label || item.value) : [];
          const hasTitle = title.display === true && titleText;
          const hasSubtitle = Boolean(subtitleText) && (subtitle.display === true || Boolean(inferredSubtitleText));
        ...
        const normalisedDatasets = sourceDatasets.map(dataset => {
          ...
          const basePointHoverRadius = Number.isFinite(dataset.pointHoverRadius) ? dataset.pointHoverRadius : basePointRadius + 2;
          const basePointHitRadius = Number.isFinite(dataset.pointHitRadius) ? dataset.pointHitRadius : 4;
        ...
        new Chart(canvas, {
          ...
          options: {
            ...
            maintainAspectRatio: Object.prototype.hasOwnProperty.call(options, 'maintainAspectRatio') ? options.maintainAspectRatio : !hasExplicitChartHeight,
            ...options,
            layout: {
              ...layout,
              padding: useDirectLabels ? mergePadding(layout.padding, directLabelLayoutPadding) : layout.padding
            },
            plugins: {
              ...plugins,
              nswDirectLabels: {
      ...
      createChart('bar', {
        type: 'bar',
        data: {
          labels: ['A thing', 'Another thing', 'Third thing', 'Fourth thing', 'Fifth thing', 'Penultimate thing', 'Last thing'],
          datasets: [{
            label: false,
      ...
      createChart('pie', {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: [125, 50, 100, 75, 50, 25, 75],
            hoverOffset: 8,
            backgroundColor: [palette.blue02, palette.teal02, palette.grey02, palette.purple02, palette.orange02, palette.brown02]
          }]
        },
        options: {
          plugins: {
      ...
      createChart('example1Bad', {
        type: 'bar',
        data: {
          labels: ['Illawarra Shoalhaven', 'Central Coast', 'Hunter New England', 'Western NSW'],
          datasets: [{
            data: [96, 101, 98, 104],
            backgroundColor: [palette.blue02]
          }]
      ...
      createChart('example4Bad', {
        type: 'pie',
        data: {
          labels: ['Category A', 'Category B', 'Category C', 'Category D', 'Category E', 'Category F', 'Category G', 'Category H', 'Category I', 'Category J', 'Category K', 'Category L'],
          datasets: [{
            data: [12, 11, 10, 9, 8, 8, 7, 7, 6, 6, 5, 4],
            backgroundColor: [palette.blue02, palette.red02, palette.green02, palette.orange02, palette.purple02, palette.teal02, palette.fuchsia02, palette.brown02, palette.blue03, palette.red03, palette.green03, palette.orange03]
          }]
        },
        options: {
          plugins: {
      ...
      createChart('example4Good', {
        ...
        data: {
          ...
          datasets: [{
            data: [46, 28, 17, 9],
            backgroundColor: context => [palette.red01, palette.red02, palette.purple01, palette.purple02][context.dataIndex]
          }]
        },
        options: {
          plugins: {
      ...
      const anatomyResolvedEarlyTotal = anatomyResolvedEarlyData.reduce((total, value) => total + value, 0);
      const anatomyResolvedLateTotal = anatomyResolvedLateData.reduce((total, value) => total + value, 0);
      ...
      createChart('chartCompBarHorizontal', {
        type: 'bar',
        data: {
          labels: ['Rental Assistance for Vulnerable Households', 'Small Business Digital Transformation Grant', 'Regional Infrastructure Maintenance Program', 'Early Childhood Development Support Initiative', 'Aboriginal Community Housing Improvement Fund', 'Domestic Violence Crisis Response Program'].map(label => wrapTickLabel(label)),
          datasets: [{
            label: 'Funding ($M)',
      ...
      createChart('chartCompGroupedHorizontal', {
        type: 'bar',
        data: {
          labels: ['Contractor licence -- building', 'Contractor licence -- electrical', 'Supervisor certificate', 'Tradesperson certificate', 'Owner-builder permit'].map(label => wrapTickLabel(label)),
          datasets: [{
            label: 'Lodged',
      ...
      createChart('chartPropPie', {
        ...
        data: {
          ...
          datasets: [{
            data: [40, 25, 20, 15],
            backgroundColor: context => [palette.blue01, palette.blue02, palette.red01, palette.red02][context.dataIndex]
          }]
        },
        options: {
          aspectRatio: 1.4,
    ...
    const waitForChartFonts = () => {
      ...
      const fontReadyPromise = Promise.all([document.fonts.load("400 16px 'Public Sans'"), document.fonts.load("600 16px 'Public Sans'")]).catch(() => {});
    ...
    const ensureChartJS = () => {
      ...
      loadScriptOnce(chartJsPrimary).then(renderChartsWhenReady).catch(() => loadScriptOnce(chartJsFallback).then(renderChartsWhenReady)).catch(err => {
